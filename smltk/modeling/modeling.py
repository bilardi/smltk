...
class Modeling:
    ...
    def prediction(self, model, method, X_test, y_test=[]):
        ...
        if len(X_test) > 0 and (
            (
                isinstance(X_test, pd.core.series.Series)
                and isinstance(X_test[X_test.first_valid_index()], tuple)
            )
            or (isinstance(X_test[:1][0], tuple))
        ):
    ...
    def predict_exists(self, model):
        ...
        Returns:
            boolean
        """
        return hasattr(model, "predict") and callable(
    ...
    def get_classification_metrics(self, params={}):
        ...
        if "model" in params:
            if self.fit_exists(params["model"]) or self.predict_exists(
                params["model"]
            ):
