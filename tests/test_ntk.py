...
class TestNtk(unittest.TestCase, Ntk):
    ...
    target = []
    tuples = []
    default_doc_filtered = (
        "good case excellent value agree mistake item match picture"
    )
    ...
    def test_get_tokens_cleaned(self):
        ...
        tokens = self.ntk_no_stop_words.get_tokens_cleaned(
            self.docs[1], is_lemma=True
        )
        self.assertEqual(tokens, ["i", "be", "agree"])
        tokens = self.ntk_no_stop_words.get_tokens_cleaned(
            self.docs[1], is_lemma=False
        )
    ...
    def test_get_doc_cleaned(self):
        self.assertEqual(
            self.ntk.get_doc_cleaned(self.doc), self.default_doc_filtered
        )
        self.assertEqual(
            self.ntk_no_stop_words.get_doc_cleaned(self.doc),
            self.without_stop_words_doc_filtered,
        )
    ...
    def test_doc_cleaned(self):
        ...
        features_token = self.ntk.create_features_from_tuples(
            self.tuples, False
        )
        self.assertEqual(
            features_token[1],
        ...
        features_token = self.ntk.create_features_from_docs(
            self.docs[:2], 1, False
        )
        features_token.extend(
            self.ntk.create_features_from_docs(self.docs[2:], 0, False)
        )
        self.assertEqual(
            features_token[1],
            (
        ...
        features_lemma = self.ntk.create_features_from_tuples(
            self.tuples, True
        )
        self.assertEqual(
            features_lemma[1],
        ...
        features_lemma = self.ntk.create_features_from_docs(
            self.docs[:2], 1, True
        )
        features_lemma.extend(
            self.ntk.create_features_from_docs(self.docs[2:], 0, True)
        )
        self.assertEqual(
            features_lemma[1],
            (
        ...
        features_lemma = self.ntk.create_features_from_docs(
            self.docs[:2], 1, True, words_top
        )
        features_lemma.extend(
            self.ntk.create_features_from_docs(
                self.docs[2:], 0, True, words_top
            )
        )
        self.assertEqual(
            features_lemma[1],
    ...
    def test_doc_cleaned_with_lemmatizer(self):
        ...
        self.assertEqual(
            ntk.get_doc_cleaned(self.doc), self.default_doc_filtered
        )

    def test_doc_cleaned_without_stop_words(self):
        vocab_token = self.ntk_no_stop_words.create_vocab_from_docs(
            self.docs, False
        )
        ...
        self.assertEqual(
            self.ntk_no_stop_words.get_stats_vocab(vocab_token), (16, 16)
        )
        self.assertEqual(
            self.ntk_no_stop_words.get_stats_vocab(vocab_token, 2), (0, 16)
        )

        vocab_lemma = self.ntk_no_stop_words.create_vocab_from_docs(
            self.docs, True
        )
        ...
        self.assertEqual(
            self.ntk_no_stop_words.get_stats_vocab(vocab_lemma), (15, 15)
        )
        self.assertEqual(
            self.ntk_no_stop_words.get_stats_vocab(vocab_lemma, 2), (1, 15)
        )

        features_token = self.ntk_no_stop_words.create_features_from_tuples(
            self.tuples, False
        )
        self.assertEqual(
            features_token[1],
        ...
        features_lemma = self.ntk_no_stop_words.create_features_from_tuples(
            self.tuples, True
        )
        self.assertEqual(
            features_lemma[1],
        ...
        for doc in self.docs:
            docs_cleaned.append(
                self.ntk_no_stop_words.get_doc_cleaned(doc, False)
            )
        tuples = self.ntk_no_stop_words.create_tuples(
            docs_cleaned, self.target
        )
        vocab_token = self.ntk_no_stop_words.create_vocab_from_tuples(
            tuples, False
        )
        self.assertEqual(
            self.ntk_no_stop_words.get_stats_vocab(vocab_token), (16, 16)
        ...
        for doc in self.docs:
            docs_cleaned.append(
                self.ntk_no_stop_words.get_doc_cleaned(doc, True)
            )
        tuples = self.ntk_no_stop_words.create_tuples(
            docs_cleaned, self.target
        )
        ...
        self.assertEqual(
            self.ntk_no_stop_words.get_stats_vocab(vocab_token), (15, 15)
        )
        self.assertEqual(
            self.ntk_no_stop_words.get_stats_vocab(vocab_token, 2), (1, 15)
        )

    def test_doc_cleaned_without_stop_words_with_min_length(self):
        ntk = Ntk({"stop_words": [], "min_length": 2})
        self.assertEqual(
            ntk.get_doc_cleaned(self.doc),
            self.without_stop_words_with_min_length_doc_filtered,
        )
    ...
    def test_doc_cleaned_without_stop_words_without_tag_map(self):
        ...
        self.assertEqual(
            ntk.get_doc_cleaned(self.doc),
            self.without_stop_words_without_tag_map_doc_filtered,
        )
    ...
    def test_get_words_top(self):
        vocabs = Counter(
            {"one": 10, "two": 8, "three": 4, "four": 2, "five": 0, "six": 1}
        )
    ...
    def test_get_vocabs_cleaned(self):
        vocabs = {
            "one": Counter({"uno": 1, "uno": 2, "hm": 3}),
            "two": Counter({"due": 2, "dos": 3, "hm": 4}),
        }
        vocabs_cleaned = self.ntk.get_vocabs_cleaned(vocabs)
        self.assertEqual(
            vocabs_cleaned,
            {
                "one": Counter({"uno": 1, "uno": 2}),
                "two": Counter({"due": 2, "dos": 3}),
            },
        )
        self.assertEqual(
            vocabs_cleaned,
            {"one": {"uno": 1, "uno": 2}, "two": {"due": 2, "dos": 3}},
        )
        vocabs = {
            "one": {"uno": 1, "uno": 2, "hm": 3},
            "two": {"due": 2, "dos": 3, "hm": 4},
        }
        vocabs_cleaned = self.ntk.get_vocabs_cleaned(vocabs)
        self.assertEqual(
            vocabs_cleaned,
    ...
    def test_get_ngrams_features(self):
        tokens = self.ntk_no_stop_words.get_tokens_cleaned(
            self.doc, is_lemma=False
        )
        features = self.ntk_no_stop_words.get_ngrams_features()
        self.assertEqual(features, {})
        tokens_features = self.ntk_no_stop_words.get_ngrams_features(
            degree=1, tokens=tokens
        )
        self.assertEqual(
            tokens_features,
            {
        ...
        doc_features = self.ntk_no_stop_words.get_ngrams_features(
            degree=1, doc=self.doc
        )
        self.assertEqual(doc_features, tokens_features)
        features = self.ntk_no_stop_words.get_ngrams_features(
            degree=1, doc=self.doc, is_lemma=True
        )
        self.assertEqual(
            features,
            {
    ...
    def test_create_ngrams_features_from_tuples(self):
        features = self.ntk.create_ngrams_features_from_tuples(
            self.tuples, True
        )
        self.assertEqual(type(features[0]), tuple)
        self.assertEqual(features[0][1], True)
    ...
    def test_get_features(self):
        ...
        features = self.ntk_no_stop_words.get_features(
            self.docs[1], is_lemma=True, degree=1
        )
        self.assertEqual(
            features,
            {
        ...
        features = self.ntk_no_stop_words.get_features(
            self.docs[1], is_lemma=False, degree=1
        )
        self.assertEqual(
            features,
            {
    ...
    def test_vectorize_docs(self):
        ...
        features_mix = data[
            ["text", "words_top", "neg", "neu", "pos", "compound"]
        ...
        X_train_tfidf = self.ntk.vectorize_docs(
            self.docs, is_count=False, is_lemma=True
        )
        self.assertEqual(type(X_train_tfidf[0]), scipy.sparse.csr_matrix)
        vectorizer_lemma = self.ntk.vectorizer
        self.assertEqual(len(vectorizer_lemma.vocabulary_), 9)
